...
<html lang="pt-BR">
	...
	<body>
		...
		//--------------- Veriifica a publicação da ressenha ---------------//
			...
			<section id="modal_resenha" class="modal_container">
				...
				<div class="modal" style="border: 0.1em solid #EDB91C;">
					...
					<div class="modal-content">
						...
						//--------------- Verifica se o modal pesquisa pela Barra ---------------//
							if($search != ""){
								$sql = "SELECT * FROM livros WHERE titulo_livro LIKE ('%$search%')";
								$result = mysqli_query($conexao,$sql);
						...
						//--------------- Verifica se o modal pesquisa pelo Gênero ---------------//
							if($genero != ""){
								$sql = "SELECT * FROM livros WHERE genero_livro = '$genero'";
								$result = mysqli_query($conexao,$sql);
		...
		function iniciaModal(modalID){
			...
			if(modal){
				...
				modal.addEventListener('click', (e) => {
					//	Removando a class 'mostrar' quando clicado no id de algum item, 
					//para tirar a visualização do modal.
